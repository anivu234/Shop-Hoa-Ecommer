...
@app.route("/admin/updateuser/<int:id>", methods=["GET", "POST"])
def admin_updateuser(id):
    if 'email' not in session:
        flash(f'Vui lòng đăng nhập trước', 'danger')
    updatebrand = Register.query.get_or_404(id)
    brand =request.form.get('Register')
    if request.method == 'POST':
        updatebrand.name =brand
        flash("User của bạn đã được cập nhật",'success')
        db.session.commit()
        return redirect(url_for('admin_user'))
    return render_template('admin/updateuser.html', updatebrand = updatebrand)


@app.route("/admin/deleteuser/<int:id>", methods=["GET", "POST"])
def admin_deleteuser(id):
    brand = Register.query.get_or_404(id)
    if request.method == "POST":
        db.session.delete(brand)
        db.session.commit()
        flash(f'User{brand.name} xóa khỏi database của bạn ','success')
        return redirect(url_for('admin_user'))
    flash(f'Thương hiệu {brand.name} có thể bị xóa', 'wraning')
    return redirect(url_for('admin_user'))
