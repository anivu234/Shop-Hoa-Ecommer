...
def twitter_login():
      ...
      return 'helo@{}'.format(account_info_json['screen_name'])
...
def twitter_logged_in(blueprint,token):
    ...
    if account_info.ok:
        ...
        query =Register.query.filter_by(username= username)
        try:
            user =query.one()
...
def check_out():
    form = FormCheckOut()
    if request.method == "POST":
        print(request.form['HoTen'])
        print(request.form['Mail'])
        print(request.form['SDT'])
        print(request.form['Dia_chi'])
        print(request.form['ThanhPho'])
        print(request.form['Ghi_chu'])
        return redirect('/success')
    ...
    if current_user.is_authenticated:
        ...
        return redirect(url_for('customerLogin'))
    return render_template('checkout.html', tax=tax, subTotal=subTotal, grandTotal=grandTotal,
                           customer=customer, orders=orders)
