from wtforms import Form, BooleanField, StringField, PasswordField, validators,IntegerField,TextAreaField,SubmitField,ValidationError,DateField
...
class FormCheckOut(FlaskForm):
    HoTen = StringField("Tên khách hàng", [validators.DataRequired("Vui lòng nhập tên")])
    Mail = StringField("Email", [validators.DataRequired("Vui lòng nhập Email"),
                                 validators.email("Phải nhập đúng cấu trúc Email")])
    SDT = StringField("Số điện thoại",
                      [validators.data_required("Vui lòng nhập số điện thoại"), validators.Length(max=11)])
    Dia_chi = StringField("Địa chỉ đặt hàng", [validators.data_required("Vui lòng nhập địa chỉ")])
    Thanh_pho = StringField("Thành phố/Tỉnh", [validators.data_required("Vui lòng nhập thành phố")])
    Ghi_chu = TextAreaField("Ghi chú")


class FormEditDon(FlaskForm):
    ngay_giao = DateField("Ngày giao", validators=[DataRequired()], format='%Y/%m/%d')